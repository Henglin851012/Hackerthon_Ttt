...
def board_topics(request, board_id):
    ...
    return render(request, 'boardtopicpage.html',
                  {'board': board, 'topics': topics, 'paginator': paginator, 'page_obj': topics,
                   'object_list': topics.object_list, 'is_paginated': topics.has_other_pages()})
...
def new_topic(request, board_id):
    ...
    if request.method == 'POST':  # if POST request ie submit button was clicked
        ...
        if form.is_valid():  # if form data has no errors
            # Create the Topic
            topic = form.save(
                commit=False)  # create a Topic model DB instance but dont save the instance since we have to do some edits
            ...
            Post.objects.create(message=form.cleaned_data.get('message'), topic=topic,
                                anonymous=form.cleaned_data.get('anonymous'), created_by=user)
    ...
    if topic.is_closed:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect('{url}?page={pageno}#{postid}'.format(url=topicurl, pageno=pageno, postid=post.id),
                                board_id=board.id, topic_id=topic.id)
        elif request.method == 'GET':
